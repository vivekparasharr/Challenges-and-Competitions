...
items_by_cat=df.groupby(['category']).nunique()[['item_id','designer']]
items_by_cat.reset_index(level=0, inplace=True) # Convert Index to Column
...
d5.reset_index(level=0, inplace=True) # Reset thrice because there were three indexes in the DataFrame
alt.Chart(items_by_cat).mark_circle(size=60).encode(
    x='item_id',
    y='designer',
    #color='objectid',
